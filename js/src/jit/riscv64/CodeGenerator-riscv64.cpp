...
// shared
CodeGeneratorRiscv64::CodeGeneratorRiscv64(MIRGenerator* gen,
                                           LIRGraph* graph,
                                           MacroAssembler* masm)
...
void CodeGeneratorRiscv64::branchToBlock(FloatFormat fmt,
                                         FloatRegister lhs,
                                         FloatRegister rhs,
                                         MBasicBlock* mir,
...
void CodeGeneratorRiscv64::emitBigIntDiv(LBigIntDiv* ins,
                                         Register dividend,
                                         Register divisor,
                                         Register output,
                                         Label* fail) {
...
void CodeGeneratorRiscv64::emitBigIntMod(LBigIntMod* ins,
                                         Register dividend,
                                         Register divisor,
                                         Register output,
                                         Label* fail) {
...
void CodeGenerator::visitExtendInt32ToInt64(LExtendInt32ToInt64* lir) {
  ...
  if (lir->mir()->isUnsigned()) {
    masm.mv(output, ToRegister(input));
  } else {
  ...
  switch (ins->bitop()) {
    ...
    case JSOp::BitXor:
      if (rhs->isConstant()) {
        ...
        masm.ma_xor(ToRegister(dest), ToRegister(lhs), Operand(ToRegister(rhs)));
        masm.slliw(ToRegister(dest), ToRegister(dest), 0);
      }
      break;
    case JSOp::BitAnd:
...
void CodeGenerator::visitPowHalfD(LPowHalfD* ins) {
  ...
  masm.CompareIsNanF32(scratch, input, input);
  masm.ma_branch(&skip, Assembler::Equal, scratch, Operand(1));
  masm.ma_compareF64(scratch, Assembler::DoubleNotEqual, input, fpscratch);
  masm.ma_branch(&skip, Assembler::Equal, scratch, Operand(1));
...
void CodeGenerator::visitWasmSelect(LWasmSelect* ins) {
  ...
  if (falseExpr->isFloatReg()) {
    if (mirType == MIRType::Float32) {
      masm.ma_fmovz(SingleFloat, out, ToFloatRegister(falseExpr),
                    cond);
    } else if (mirType == MIRType::Double) {
      masm.ma_fmovz(DoubleFloat, out, ToFloatRegister(falseExpr),
                    cond);
    } else {
...
void CodeGenerator::visitWasmReinterpret(LWasmReinterpret* lir) {
  ...
  switch (to) {
    case MIRType::Int32:
      ...
      masm.fmv_x_w(ToRegister(lir->output()),
                         ToFloatRegister(lir->input()));
      break;
    case MIRType::Float32:
      MOZ_ASSERT(from == MIRType::Int32);
      masm.fmv_w_x(ToFloatRegister(lir->output()),
