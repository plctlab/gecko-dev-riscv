...
class CodeGeneratorRiscv64 : public CodeGeneratorShared {
 ...
 protected:
  CodeGeneratorRiscv64(MIRGenerator* gen,
                       LIRGraph* graph,
                       MacroAssembler* masm);
  ...
  void bailoutCmp32(Assembler::Condition c,
                    T1 lhs,
                    T2 rhs,
                    LSnapshot* snapshot) {
    Label bail;
  ...
  template <typename T1, typename T2>
  void bailoutTest32(Assembler::Condition c,
                     T1 lhs,
                     T2 rhs,
                     LSnapshot* snapshot) {
    Label bail;
  ...
  template <typename T1, typename T2>
  void bailoutCmpPtr(Assembler::Condition c,
                     T1 lhs,
                     T2 rhs,
                     LSnapshot* snapshot) {
    Label bail;
  ...
  void bailoutTestPtr(Assembler::Condition c,
                      Register lhs,
                      Register rhs,
                      LSnapshot* snapshot) {
  ...
  template <typename T>
  void branchToBlock(Register lhs,
                     T rhs,
                     MBasicBlock* mir,
                     Assembler::Condition cond) {
    masm.ma_b(lhs, rhs, skipTrivialBlocks(mir)->lir()->label(), cond);
  }
  void branchToBlock(FloatFormat fmt,
                     FloatRegister lhs,
                     FloatRegister rhs,
                     MBasicBlock* mir,
  ...
  void testZeroEmitBranch(Assembler::Condition cond,
                          Register reg,
                          MBasicBlock* ifTrue,
                          MBasicBlock* ifFalse) {
  ...
  void emitTableSwitchDispatch(MTableSwitch* mir,
                               Register index,
                               Register base);
  ...
  void atomicBinopToTypedIntArray(AtomicOp op,
 ...
 protected:
  ...
  void emitBigIntDiv(LBigIntDiv* ins,
                     Register dividend,
                     Register divisor,
                     Register output,
                     Label* fail);
  void emitBigIntMod(LBigIntMod* ins,
                     Register dividend,
                     Register divisor,
                     Register output,
                     Label* fail);
