 ...
 * CodeGeneratorriscv64::generateOutOfLineCode()) and thunk code should save it
 * on stack. Other difference is that members snapshotOffset_ and padding_ are
 * pushed to the stack by CodeGeneratorriscv64::visitOutOfLineBailout(). Field
...
JitRuntime::BailoutTable JitRuntime::generateBailoutTable(MacroAssembler& masm,
                                                          Label* bailoutTail,
                                                          uint32_t frameClass) {
  MOZ_CRASH("riscv64 does not use bailout tables");
}
  ...
  for (uint32_t explicitArg = 0; explicitArg < f.explicitArgs; explicitArg++) {
    switch (f.argProperties(explicitArg)) {
      ...
      case VMFunctionData::DoubleByRef:
        MOZ_CRASH("NYI: riscv64 callVM should not be used with 128bits values.");
        break;
    }
  }
